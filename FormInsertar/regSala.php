    ...
    <main class="col-md-9 ms-sm-auto col-lg-10 px-md-4 mt-4">
      <div class="card shadow-sm mx-auto" style="max-width: 700px;">
        <div class="card-header bg-primary text-white d-flex align-items-center">
          ...
          <h5 class="mb-0">Registro de Sala</h5>
        </div>

        <div class="card-body">
            <form action="phpInsertar/gdSala.php" method="post">
                <div class="mb-3">
                  <label for="titulo" class="form-label">Ingresa la Sala</label>
                  <input type="text" class="form-control" id="titulo" name="d1" required>
                </div>

                <div class="mb-3">
                  <label for="sala" class="form-label">Tipo</label>
                  <select id="sala" name="d2" class="form-select" required>
                    <option value="">Seleccione un tipo</option>
                    <?php
                        // Conexión a la base de datos
                        $conn = mysqli_connect("localhost", "root", "", "cine");
                        $sql = "SELECT id, nombre FROM tipossalas";
